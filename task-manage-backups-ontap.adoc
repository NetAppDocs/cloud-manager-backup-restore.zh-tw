...
+
您可以使用「自動備份未來的磁碟區...」核取方塊、選擇是否要在未來新增所有磁碟區以啟用備份。如果停用此設定、您日後需要手動啟用新增磁碟區的備份。
...
+
如果您的叢集執行ONTAP 的是版本不支援的版本號（例如更新版本的版本）、而且您使用AWS或Azure來儲存雲端、您也可以選擇在特定天數後、啟用或停用將備份分層至歸檔儲存設備的功能。

+
link:reference-azure-backup-tiers.html["深入瞭解如何使用Azure歸檔儲存設備"]。link:reference-aws-backup-tiers.html["深入瞭解使用AWS歸檔儲存設備"]。

+
image:screenshot_backup_modify_policy_page2.png["這是一份快照、顯示雲端備份的歸檔儲存設定分層。"]

+
請注意、如果您停止分層備份至歸檔儲存設備、任何已分層至歸檔儲存設備的備份檔案都會留在該層中、而不會自動移回標準層級。
