...
雲端還原執行個體部署在雲端時、其所在的子網路與Connector相同。
...
需要AWS還原權限:: 提供Cloud Manager權限的IAM角色需要下列EC2權限、以便能夠啟動、停止及終止Cloud Restore執行個體以進行瀏覽與還原作業：
+
--
[source, json]
----
...
----
--
AWS部署所需的傳出網際網路存取:: Cloud Restore執行個體需要傳出網際網路存取。如果您的虛擬或實體網路使用Proxy伺服器進行網際網路存取、請確定執行個體具有傳出網際網路存取權限、以聯絡下列端點。
+
--
[cols="43,57"]
|===
